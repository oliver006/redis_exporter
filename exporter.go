...
func NewRedisExporter(redisURI string, opts Options) (*Exporter, error) {
	...
	for k, desc := range map[string]struct {
		...
		"stream_groups_count":                  {txt: `Groups count of stream`, lbls: []string{"db", "stream"}},
		"stream_group_consumers_count":         {txt: `Consumers count of stream group`, lbls: []string{"db", "stream", "group"}},
		"stream_group_pending_count":           {txt: `Pending items count of stream group`, lbls: []string{"db", "stream", "group"}},
...
func (e *Exporter) extractStreamMetrics(ch chan<- prometheus.Metric, c redis.Conn) {
	...
	for _, k := range allKeys {
		...
		e.registerConstMetricGauge(ch, "stream_groups_count", float64(info.Groups), dbLabel, k.key)
		for _, g := range info.StreamGroupsInfo {
			e.registerConstMetricGauge(ch, "stream_group_consumers_count", float64(g.Consumers), dbLabel, k.key, g.Name)
			e.registerConstMetricGauge(ch, "stream_group_pending_count", float64(g.Pending), dbLabel, k.key, g.Name)
		}
