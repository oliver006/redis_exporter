...
| client-idle-threshold-seconds       | REDIS_EXPORTER_CLIENT_IDLE_THRESHOLD_SECONDS     | Export a low-cardinality count (`idle_clients`) of clients whose idle time is greater than or equal to this threshold in seconds (via CLIENT LIST). Excludes pub/sub (`P`), blocked (`b`), replication (`M`, `S`), MONITOR (`O`), MULTI/EXEC (`x`), WATCH-modified (`d`), cluster slot migration (`g`, `o`), client-side caching tracking (`t`, `T`, `B`, `R`), and Sentinel (`cmd` prefix `sentinel\|`). See also `pubsub_clients`, `blocked_clients`, and `tracking_clients`. Defaults to 0 (disabled). Independent of `--export-client-list`. Requires `CLIENT` ACL permission.                                                                                                                                                                                                                                                                                                                                                                              |
