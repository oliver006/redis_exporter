...
| client-idle-threshold-seconds       | REDIS_EXPORTER_CLIENT_IDLE_THRESHOLD_SECONDS     | Export a low-cardinality count (`idle_clients`) of clients whose idle time is greater than or equal to this threshold in seconds (via CLIENT LIST). Excludes client types that Redis/Valkey does not idle-timeout: pub/sub (`P`), blocked (`b`), master (`M`), replica (`S`), MONITOR (`O`), and cluster slot migration (`g`, `o`). See also `pubsub_clients` and `blocked_clients`. Defaults to 0 (disabled). Independent of `--export-client-list`. Requires `CLIENT` ACL permission.                                                                                                                                                                                                                                                                                                                                                                              |
