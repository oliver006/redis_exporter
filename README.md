...
| client-idle-threshold-seconds       | REDIS_EXPORTER_CLIENT_IDLE_THRESHOLD_SECONDS     | Export a low-cardinality count (`client_connections_idle`) of clients whose idle time is greater than or equal to this threshold in seconds (via CLIENT LIST). Pub/Sub and blocked clients are excluded (see `pubsub_clients` and `blocked_clients`). Defaults to 0 (disabled). Independent of `--export-client-list`. Requires `CLIENT` ACL permission.                                                                                                                                                                                                                                                                                                                                                                              |
