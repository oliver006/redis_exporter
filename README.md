...
Target-based discovery is **disabled by default** so the exporter never connects to caller-supplied addresses unless you opt in. Enable it by setting `--cluster-discover-target-allowlist` (or `REDIS_EXPORTER_CLUSTER_DISCOVER_TARGET_ALLOWLIST`) to a comma-separated list of host globs; only a `target` whose host matches one of the globs is accepted, anything else returns `403`. ElastiCache hostnames contain a fixed string shared by all clusters in your account, so one glob covers them all, e.g. `--cluster-discover-target-allowlist='clustercfg.*.abcdef.apse2.cache.amazonaws.com'`.

Patterns are host globs ([`path.Match`](https://pkg.go.dev/path#Match) syntax) matched against the literal target host, not CIDR ranges — a pattern like `10.0.0.0/24` matches nothing (the `/` is a glob path separator), so use literal IPs or wildcard octets such as `10.0.0.*` if you need to allow IP targets.
...
| cluster-discover-hostnames          | REDIS_EXPORTER_CLUSTER_DISCOVER_HOSTNAMES        | Whether to use hostname for cluster node discovery if available via `/discover-cluster-nodes` endpoint, defaults to false.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| cluster-discover-target-allowlist   | REDIS_EXPORTER_CLUSTER_DISCOVER_TARGET_ALLOWLIST | Comma-separated list of host globs ([`path.Match`](https://pkg.go.dev/path#Match) syntax) allowed as the `target` parameter of `/discover-cluster-nodes`. Empty (default) disables target-based discovery, so the exporter will not connect to caller-supplied addresses.                                                                                                                                                                                                                                                                                                                                                                        |
...
| tls-server-name                     | REDIS_EXPORTER_TLS_SERVER_NAME                   | Server name used to verify the Redis server certificate when the exporter connects to a Redis instance over TLS. Also sets the TLS SNI server name.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
If the Redis address does not match the certificate hostname, set `--tls-server-name` or pass `tls_server_name` to the `/scrape` endpoint.
