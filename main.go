...
func main() {
	var (
		...
		clientIdleThresholdSeconds     = flag.Int64("client-idle-threshold-seconds", getEnvInt64("REDIS_EXPORTER_CLIENT_IDLE_THRESHOLD_SECONDS", 0), "Export a low-cardinality count of clients whose idle time (from CLIENT LIST) is greater than or equal to this threshold in seconds. Excludes pub/sub and blocked clients. 0 disables this metric.")
	...
	exp, err := exporter.NewRedisExporter(
		*redisAddr,
		exporter.Options{
			...
			ClientIdleThresholdSeconds:     *clientIdleThresholdSeconds,
