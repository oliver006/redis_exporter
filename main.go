...
func main() {
	var (
		...
		inclCommandStats               = flag.Bool("include-command-stats", getEnvBool("REDIS_EXPORTER_INCL_COMMAND_STATS", false), "Whether to include per-command metrics (calls, duration) by fetching INFO commandstats separately. Enables redis_commands_total{cmd=...} metrics.")
	...
	exp, err := exporter.NewRedisExporter(
		*redisAddr,
		exporter.Options{
			...
			InclCommandStats:               *inclCommandStats,
