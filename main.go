...
func main() {
	var (
		...
		checkKeyGroupsBatchSize = flag.Int64("check-key-groups-batch-size", getEnvInt64("REDIS_EXPORTER_CHECK_KEY_GROUPS_BATCH_SIZE", 10000), "Check key groups batch size hint for the underlying SCAN, larger value speeds up scanning.\nWARNING: Still Redis is a single-threaded app, huge COUNT can affect production environment.")
